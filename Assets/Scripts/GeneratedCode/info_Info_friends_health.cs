...
{
	public class info_Info_friends_health : Sensor
	{
		private int counter;
		...
		public override void Initialize(SensorConfiguration sensorConfiguration)
		{
			...
			counter = 0;
			mappingTemplate = "info_Info_friends_health(info_object,objectIndex("+index+"),{1},{0})." + Environment.NewLine;
		}
		...
		public override void Update()
		{
			...
			if(!invariant || first)
			{
				...
				List<int> friends_health_2 = Info_1.friends_health;
				if(friends_health_2 == null)
				{
					values.Clear();
					return;
				}
				else if(friends_health_2.Count > values.Count)
				{
					for(int i = values.Count; i < friends_health_2.Count; i++)
					{
				...
				{
					for(int i = friends_health_2.Count; i < values.Count; i++)
					{
						values.RemoveAt(values.Count - 1);
					}
				}
				for(int i_2 = 0; i_2 < friends_health_2.Count; i_2++)
				{
					if (values[i_2].Count == 1)
					{
						values[i_2].RemoveAt(0);
					}
					values[i_2].Add(friends_health_2[i_2]);
				}
			}
		}
