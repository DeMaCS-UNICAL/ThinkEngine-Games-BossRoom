...
namespace Unity.BossRoom.Gameplay.GameplayObjects.Character
{
    ...
    public class ServerCharacter : NetworkBehaviour, ITargetable
    {
        ...
        public bool basic_attack_hit=false;
        ...
        public override void OnNetworkSpawn()
        {
         ...
         public NetworkAvatarGuidState GetState(){
            return m_State;
        }

        public ServerCharacterMovement GetMovement(){
            return m_Movement;
        }

        //id of the last action performed by the player
        public int my_current_action_ID=-1;

        //returns the player's health percentage based on his class
        public int getHealthPercent(){
            
            int temp = HitPoints * 100;
            if(CharacterClass.CharacterType == CharacterTypeEnum.Archer){
                temp = temp/900;
            }
            else if(CharacterClass.CharacterType == CharacterTypeEnum.Mage){
                temp = temp/800;
            }
            else if(CharacterClass.CharacterType == CharacterTypeEnum.Rogue){
                temp = temp/1000;
            }
            else{
                temp = temp/1200;
