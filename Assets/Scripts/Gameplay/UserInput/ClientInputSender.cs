...
namespace Unity.BossRoom.Gameplay.UserInput
{
    ...
    public class ClientInputSender : NetworkBehaviour
    {
        ...
        //at the next update the player will move in m_x and m_z
        public float m_x; 
        public float m_z;
        ...
        //this function is invoked by the AI ​​to attack a target
        public void setTarget(GameObject target)
        {   
            //if the player already has a target he changes it with the one passed in the function
            if(this.m_TargetServerCharacter != null && target!=null)
                this.m_TargetServerCharacter = target.GetComponent<ServerCharacter>();

            //the archer is a bit buggy and doesn't stop if he has to attack
            if(this.CharacterClass.CharacterType == CharacterTypeEnum.Archer && 
                m_ServerCharacter.GetMovement().IsMoving())
            {
                m_ServerCharacter.GetMovement().CancelMove();
            }

            //if the target passed to the function is still alive, the attack ability is invoked
            if(target!=null)
                PerformSkill(CharacterClass.Skill1.ActionID, SkillTriggerStyle.Keyboard, 
                target.GetComponent<NetworkObject>().NetworkObjectId);
            
        }

        //function invoked by the AI if is a mage to heal himself or an ally
        public void healTarget(GameObject target){
            //if the player already has a target he changes it with the one passed in the function
            if(this.m_TargetServerCharacter != null && target!=null)
                this.m_TargetServerCharacter = target.GetComponent<ServerCharacter>();
            
            //if the target passed to the function is still alive, the heal skill is invoked
            if(target!=null)
                PerformSkill(CharacterClass.Skill2.ActionID, SkillTriggerStyle.Keyboard, 
                target.GetComponent<NetworkObject>().NetworkObjectId);
        }

        //this function is invoked by the AI ​​to revive aa ally
        public void reviveTarget(GameObject target){
            //if the player already has a target he changes it with the one passed in the function
            if(this.m_TargetServerCharacter != null && target!=null)
                this.m_TargetServerCharacter = target.GetComponent<ServerCharacter>();
            
            if(target!=null)
                PerformSkill(CharacterClass.Skill1.ActionID, SkillTriggerStyle.MouseClick, 
                target.GetComponent<NetworkObject>().NetworkObjectId);
        }

        public bool clearTarghet(){
            this.m_TargetServerCharacter=null;
            return true;
        }
